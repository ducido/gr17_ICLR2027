...
LIBERO-plus (https://github.com/sylvestf/LIBERO-plus) vendors the ``libero``
package name (see its ``setup.py``), so this module is a near-identical twin
of ``gr00t.eval.sim.LIBERO.libero_env`` — same observation/action-space
conversion logic — kept as a *separate* module (rather than reusing the stock
one) so gym registration, the entry_point, and the isolated uv venv are all
unambiguous about which benchmark is running. Registered under the
``libero_plus_sim/`` prefix (distinct from stock LIBERO's ``libero_sim/``) so
``gr00t.eval.sim.env_utils`` can dispatch to this module instead of the stock
one even though both share ``EmbodimentTag.LIBERO_PANDA``.
"""
...
def register_libero_plus_envs():
    ...
    for task_suite_name in [
        ...
        for task_id in range(task_suite.get_num_tasks()):
            ...
            register(
                id=f"libero_plus_sim/{task_name}",
