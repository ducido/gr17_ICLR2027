...
def calc_mse_for_single_trajectory(
    ...
    for step_count in range(steps):
        ...
        if plot_state:
            ...
            data_point = extract_step_data(
                episode_data, step_index=step_count, modality_configs=modality_config, embodiment_tag='new_embodiment', allow_padding=False
            )
        ...
        if step_count % action_horizon == 0:
            ...
            observation = {
                "video": {k: np.stack(data_point.images[k])[None] for k in data_point.images},  # stack images and add batch dimension
                "state": {k: data_point.states[k][None] for k in data_point.states},  # add batch dimension
                "action": {k: data_point.actions[k][None] for k in data_point.actions},  # add batch dimension
                "language": {
                    modality_config["language"].modality_keys[0]: [[data_point.text]],  # add time and batch dimension
                }
            }
            ...
            for j in range(action_horizon):
                ...
                concat_pred_action = np.concatenate([np.atleast_1d(action_chunk[f"{key}"][0][j]) for key in modality_keys], axis=0)
                pred_action_across_time.append(concat_pred_action)

                concat_gt_action = np.concatenate(
                    [observation['action'][key][0][j] for key in modality_keys], axis=0
                )
    ...
    if plot or save_plot_path is not None:
        ...
        plot_trajectory(info, save_plot_path=f"{save_plot_path}.png")
        
        pred_action_np = np.array(pred_action_across_time)
        print("action_shape:", pred_action_np.shape)
        np.save(f"{save_plot_path}.npy", pred_action_np)
    return mse
